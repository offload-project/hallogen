...
const ComboBoxInput = (props: InputProps) => {
  ...
  return (
    <span data-slot="control" className="relative isolate block has-[svg:last-child]:[&_input]:pe-10">
      <Input {...props} placeholder={props?.placeholder} />
      <Button className="absolute end-0 top-0 grid h-full w-11 cursor-default place-content-center sm:w-9">
        {!context?.inputValue && (
          <ChevronsUpDownIcon data-slot="chevron" className="-me-1 size-5 text-muted-fg sm:size-4" />
        )}
      </Button>
    </span>
  );
