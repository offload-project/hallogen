...
import { Link, type LinkProps, type LinkRenderProps } from "./link";
...
const SidebarProvider = ({
  ...
  const setOpen = useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      // biome-ignore lint/suspicious/noDocumentCookie: set sidebar state cookie
    ...
    <SidebarContext value={contextValue}>
      <div
        ...
        className={twMerge(
          ...
          "group/sidebar-root peer/sidebar-root has-data-[variant=inset]:bg-sidebar dark:has-data-[variant=inset]:bg-bg",
          className,
        )}
        ref={ref}
        {...props}>
...
const Sidebar = ({
  ...
  if (isMobile) {
    return (
      <>
        <span className="sr-only" aria-hidden data-variant={variant} />
        <SheetContent
    ...
    <div
      ...
      <div
        ...
        className={twMerge([
          ...
          variant === "default" && "group-data-[collapsible=dock]:w-(--sidebar-width-dock)",
          variant === "float" && "group-data-[collapsible=dock]:w-[calc(var(--sidebar-width-dock)+(--spacing(4)))]",
          variant === "inset" && "group-data-[collapsible=dock]:w-[calc(var(--sidebar-width-dock)+(--spacing(2)))]",
        ])}
      />
      <div
        ...
        className={twMerge(
          ...
          variant === "float" && "bg-bg p-2 group-data-[collapsible=dock]:w-[calc(--spacing(4)+2px)]",
          variant === "inset" &&
        ...
        <div
          ...
          className={twJoin(
            ...
            "group-data-[variant=float]:rounded-lg group-data-[variant=float]:border group-data-[variant=float]:border-sidebar-border group-data-[variant=float]:bg-sidebar group-data-[variant=float]:shadow-xs",
          )}>
          {children}
        </div>
      </div>
...
const SidebarHeader = ({ className, ref, ...props }: ComponentProps<"div">) => {
  ...
  return (
    <div
      ...
      className={twMerge([
        ...
        "in-data-[variant=inset]:px-6 in-data-[variant=inset]:py-4",
        className,
      ])}
      {...props}
    />
...
const SidebarSectionGroup = ({ className, ...props }: ComponentProps<"section">) => {
...
interface SidebarItemProps extends Omit<ComponentProps<typeof Link>, "children"> {
  isCurrent?: boolean;
  children?:
    | ReactNode
...
const SidebarItem = ({ isCurrent, tooltip, children, badge, className, ref, ...props }: SidebarItemProps) => {
  ...
  const link = (
    <Link
      ...
      className={composeRenderProps(className, (className, { isFocusVisible, isPressed, isHovered, isDisabled }) =>
        twMerge(
          ...
          isPressed && "bg-sidebar-accent text-sidebar-accent-fg [&_svg:not([class*='text-'])]:text-sidebar-accent-fg",
          isHovered && "bg-sidebar-accent text-sidebar-accent-fg [&_svg:not([class*='text-'])]:text-sidebar-accent-fg",
...
const SidebarInset = ({ className, ref, ...props }: ComponentProps<"main">) => {
  return (
    <main
      ...
      className={twMerge(
        ...
        "group-has-data-[variant=inset]/sidebar-root:border group-has-data-[variant=inset]/sidebar-root:border-sidebar-border group-has-data-[variant=inset]/sidebar-root:bg-muted",
        "md:group-has-data-[variant=inset]/sidebar-root:m-2",
        "md:group-has-data-[side=left]:group-has-data-[variant=inset]/sidebar-root:ms-0",
        "md:group-has-data-[side=right]:group-has-data-[variant=inset]/sidebar-root:me-0",
        "md:group-has-data-[variant=inset]/sidebar-root:rounded-2xl",
        "md:group-has-data-[variant=inset]/sidebar-root:peer-data-[state=collapsed]:ms-2",
        className,
      )}
      {...props}
    />
...
const SidebarDisclosureGroup = ({
  ...
  return (
    <DisclosureGroup
      ...
      className={cx("col-span-full flex min-w-0 flex-col gap-y-0.5 in-data-[state=collapsed]:gap-y-1.5", className)}
      {...props}
    />
  );
};
  ...
  return (
    <Heading level={3}>
      <Trigger
        ref={ref}
        slot="trigger"
        className={composeRenderProps(className, (className, { isPressed, isFocusVisible, isHovered, isDisabled }) =>
          twMerge(
...
const SidebarTrigger = ({ onPress, className, children, ...props }: ComponentProps<typeof Button>) => {
...
const SidebarRail = ({ className, ref, ...props }: ComponentProps<"button">) => {
...
const SidebarLabel = ({ className, ref, ...props }: ComponentProps<typeof Text>) => {
  const { state, isMobile } = useSidebar();
  const collapsed = state === "collapsed" && !isMobile;
  if (!collapsed) {
    return (
  ...
  return (
    <nav
      ...
      className={twMerge(
        ...
        isSticky && "static top-0 z-40 group-has-data-[variant=default]/sidebar-root:sticky",
        className,
      )}
      {...props}
    />
