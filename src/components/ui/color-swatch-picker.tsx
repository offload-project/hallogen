import type { ColorSwatchPickerItemProps, ColorSwatchPickerProps } from "react-aria-components/ColorSwatchPicker";
import {
...
export function ColorSwatchPickerItem({ children, className, ...props }: ColorSwatchPickerItemProps) {
  return (
    <PrimitiveColorSwatchPickerItem
      style={({ defaultStyle }) => ({
        ...defaultStyle,
        "--tw-ring-color": props.color ? `color-mix(in oklab, ${props.color} 40%, transparent)` : undefined,
      })}
      className={cx(
