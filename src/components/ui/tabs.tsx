import { type ComponentProps, createContext, type RefObject, use } from "react";
...
const TabList = <T extends object>({ className, selectionIndicator = true, ref, ...props }: TabListProps<T>) => {
  return (
...
export function TabScrollArea({ className, ...props }: ComponentProps<"div">) {
  return (
    <div className="relative">
      <div className={twMerge("scrollbar-hidden overflow-x-auto sm:overflow-x-visible", className)}>
        <div className="pointer-events-none absolute inset-x-0 bottom-0 h-px w-full bg-border" aria-hidden />
        {props.children}
      </div>
    </div>
  );
