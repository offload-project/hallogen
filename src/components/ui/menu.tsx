...
const MenuItem = ({ className, variant, children, ...props }: MenuItemProps) => {
      ...
      className={composeRenderProps(className, (className, { hasSubmenu, ...renderProps }) =>
        dropdownItemStyles({
          ...
          className: hasSubmenu
            ? twMerge(
                variant === "danger" && "open:bg-danger-subtle open:text-danger-subtle-fg",
                variant === "warning" && "open:bg-warning-subtle open:text-warning-subtle-fg",
                variant === undefined &&
...
const MenuSection = <T extends object>({ className, children, ref, ...props }: MenuSectionProps<T>) => {
  return (
    <MenuSectionPrimitive data-slot="menu-section" ref={ref} className={section({ className })} {...props}>
