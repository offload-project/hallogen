...
import { type ComponentProps, createContext, use, useEffect } from "react";
  ...
  return (
    <SearchField
      ...
      {isPending ? (
        ...
        <SearchIcon data-slot="command-menu-search-icon" className="size-5 shrink-0 text-muted-fg" />
      )}
      <Input
...
const CommandMenuSection = <T extends object>({ className, ref, ...props }: MenuSectionProps<T>) => {
  return (
    <MenuSection
      ref={ref}
      className={twMerge("col-span-full grid grid-cols-[auto_1fr] content-start gap-y-px", className)}
      {...props}>
      {"label" in props && (
...
const CommandMenuItem = ({ className, ...props }: ComponentProps<typeof MenuItem>) => {
  const textValue = props.textValue || (typeof props.children === "string" ? props.children : undefined);
  return <MenuItem {...props} textValue={textValue} className={cx("items-center gap-y-0.5", className)} />;
};

interface CommandMenuDescriptionProps extends ComponentProps<typeof MenuDescription> {}

const CommandMenuDescription = ({ className, ...props }: CommandMenuDescriptionProps) => {
  return <MenuDescription className={twMerge("col-start-3 row-start-1 ms-auto", className)} {...props} />;
};

const renderer: CollectionRenderer = {
  CollectionRoot(props) {
    if (props.collection.size === 0) {
      return <div className="col-span-full p-4 text-center text-muted-fg text-sm">No results found.</div>;
    }
...
const CommandMenuSeparator = ({ className, ...props }: ComponentProps<typeof MenuSeparator>) => (
  <MenuSeparator className={twMerge("-mx-2", className)} {...props} />
);

const CommandMenuFooter = ({ className, ...props }: ComponentProps<"div">) => {
  return (
    <div
      className={twMerge(
...
const CommandMenuShortcut = ({ className, ...props }: ComponentProps<typeof DropdownKeyboard>) => (
  <DropdownKeyboard
    className={twMerge(
