...
import { type ComponentProps, createContext, type RefObject, use, useCallback, useId, useMemo, useState } from "react";
...
const Navbar = ({
  ...
  return (
    <div
      ...
      data-navbar-sticky={isSticky}
      className={twMerge([
        "group/navbar-variant relative isolate",
        isSticky && "sticky top-0 z-40",
        placement === "top" && variant === "float" && "md:pt-8",
        placement === "bottom" && variant === "float" && "bottom-0 md:pb-8",
        variant === "float" && "mx-auto w-full max-w-7xl px-4 xl:max-w-(--breakpoint-xl)",
      ])}
      {...props}>
      <div
        className={twMerge(
          ...
          ["default", "inset"].includes(variant) && "px-4",
          variant === "default" && "border-b bg-navbar",
          className,
        )}>
        <div data-navbar="content" className="mx-auto w-full max-w-(--breakpoint-2xl) items-center md:flex">
          {children}
        </div>
      </div>
    </div>
          ...
          {(isCurrent || values.isCurrent) && (
            <motion.span
              ...
              className={twJoin(
                ...
                "-inset-s-4 inset-y-[calc(var(--navbar-gutter)-(--spacing(0.5)))] w-(--gutter) md:inset-y-auto md:w-auto",
...
const NavbarSpacer = ({ className, ref, ...props }: ComponentProps<"div">) => {
  return <div ref={ref} className={twMerge("-ms-4 flex-1", className)} {...props} />;
};

const NavbarStart = ({ className, ref, ...props }: ComponentProps<"div">) => {
  return <div ref={ref} className={twMerge("relative p-2 py-4 md:p-0.5", className)} {...props} />;
};

const NavbarGap = ({ className, ref, ...props }: ComponentProps<"div">) => {
...
const NavbarMobile = ({ className, ref, ...props }: ComponentProps<"div">) => {
  return (
    <div
      ref={ref}
...
const NavbarInset = ({ className, ref, children, ...props }: ComponentProps<"div">) => {
  return (
    <div
      ref={ref}
...
const NavbarLabel = ({ className, ...props }: ComponentProps<"span">) => {
  return <span data-slot="navbar-label" className={twJoin("col-start-2 row-start-1 truncate", className)} {...props} />;
};
