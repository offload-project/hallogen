...
export function MeterTrack({ className, ...props }: ComponentProps<"div">) {
  ...
  return (
    <div
      ...
      <div
        ...
        className="absolute inset-s-0 top-0 h-full rounded-full transition-[width] duration-200 ease-linear will-change-[width] motion-reduce:transition-none forced-colors:bg-[Highlight]"
...
export function MeterValue({ className, ...props }: Omit<ComponentProps<"span">, "children">) {
  const { valueText } = use(MeterContext)!;
  return (
    <span data-slot="meter-value" className={twMerge("text-base/6 sm:text-sm/6", className)} {...props}>
      {valueText}
    </span>
  );
}

export function MeterHeader({ className, ...props }: ComponentProps<"div">) {
  return (
    <div data-slot="meter-header" className={twMerge("flex items-center justify-between", className)} {...props} />
  );
}
