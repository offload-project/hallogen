...
interface BarListProps<T = unknown> extends ComponentProps<"div"> {
  data: Bar<T>[];
            ...
            <div
              ...
              <div className="absolute start-2 flex max-w-full pe-3 sm:pe-2">
                {item.href ? (
                  ...
                  <p className="truncate whitespace-nowrap text-base/6 text-fg sm:text-sm/6">{item.name}</p>
                )}
              </div>
            </div>
          </Component>
      ...
      <div>
        {sortedData.map((item, index) => (
          <div
            ...
            <p className="truncate whitespace-nowrap text-fg text-sm leading-none">{valueFormatter(item.value)}</p>
          </div>
        ))}
      </div>
    </div>
