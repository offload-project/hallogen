import { type ComponentProps, type Ref, use } from "react";
...
export function DisclosureTrigger({ ref, className, triggerIndicator = true, ...props }: DisclosureTriggerProps) {
  ...
  return (
    <Heading>
      <Button
        ...
        className={cx(
          [
            "relative isolate flex w-full cursor-default items-center justify-between px-(--disclosure-gutter-x,--spacing(0)) py-[calc(var(--disclosure-gutter-x,--spacing(0))-(--spacing(1)))] text-start font-medium outline-hidden sm:text-sm/6",
...
export function DisclosureIndicator({ className, ...props }: ComponentProps<"span">) {
  return (
    <span
