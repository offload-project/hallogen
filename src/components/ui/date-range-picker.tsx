...
export interface DateRangePickerProps<T extends DateValue> extends DateRangePickerPrimitiveProps<T> {
...
export function DateRangePickerTrigger({ className, ...props }: ComponentProps<typeof InputGroup>) {
  return (
    <InputGroup
      className={cx(
        "inline-flex items-center overflow-hidden rounded-lg pe-6",
...
function DateInput({ className, ...props }: ComponentProps<typeof PrimitiveDateInput>) {
  return <PrimitiveDateInput className={cx("rounded-none border-none focus-within:ring-0", className)} {...props} />;
}
