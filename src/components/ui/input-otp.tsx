...
import { type ComponentProps, type ComponentPropsWithoutRef, use } from "react";
import { twMerge } from "tailwind-merge";
import { fieldStyles } from "@/components/shared/field-styles.ts";
import { Label } from "@/components/ui/field";

export function InputOTP({ containerClassName, ...props }: ComponentPropsWithoutRef<typeof OTPInput>) {
  return (
    <span data-slot="control" className="relative block">
      <OTPInput
        data-slot="input-otp"
        containerClassName={twMerge(fieldStyles({ className: "has-[:disabled]:opacity-50" }), containerClassName)}
        {...props}
      />
    </span>
  );
...
export function InputOTPControl({ className, ...props }: ComponentProps<"span">) {
  return (
    <span
      data-slot="control"
...
export function InputOTPGroup({ className, ...props }: ComponentProps<"div">) {
  return <div data-slot="input-otp-group" className={twMerge("flex items-center", className)} {...props} />;
}
...
export function InputOTPLabel(props: ComponentProps<typeof Label>) {
