...
export function NativeSelect({ className, ...props }: ComponentProps<"div">) {
  return (
    <div
      data-slot="control"
...
export interface NativeSelectContentProps extends ComponentProps<"select"> {
  isInvalid?: boolean;
}
  ...
  return (
    <div data-slot="control" className="relative flex items-center justify-between">
      <select
        ...
        className={twMerge(
          "relative block w-full appearance-none rounded-lg bg-(--control-bg,transparent) px-[calc(--spacing(3.5)-1px)] py-[calc(--spacing(2.5)-1px)] sm:py-[calc(--spacing(1.5)-1px)] sm:pr-8 sm:pl-[calc(--spacing(3)-1px)]",
        ...
        {...props}
      />
      <ChevronsUpDownIcon className="pointer-events-none absolute top-1/2 right-3 size-5 -translate-y-1/2 text-muted-fg sm:right-2.5 sm:size-4" />
    </div>
  );
}
