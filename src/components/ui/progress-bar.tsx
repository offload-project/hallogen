...
export function ProgressBarHeader({ className, ...props }: ComponentProps<"div">) {
  return (
    <div
...
export function ProgressBarValue({ className, ...props }: Omit<ComponentProps<"span">, "children">) {
  const { valueText } = use(ProgressBarContext)!;
  return (
    <span data-slot="progress-bar-value" className={twMerge("text-base/6 sm:text-sm/6", className)} {...props}>
      {valueText}
    </span>
  );
}

export function ProgressBarTrack({ className, ref, ...props }: ComponentProps<"div">) {
