...
function toUrl(href: NonNullable<InertiaLinkProps["href"]>): string {
  return isUrlMethodPair(href) ? href.url : href;
}
...
export function useCurrentUrl(): UseCurrentUrlReturn {
  const page = usePage();
  const currentUrlPath = new URL(page.url, typeof window !== "undefined" ? window.location.origin : "http://localhost")
    .pathname;
